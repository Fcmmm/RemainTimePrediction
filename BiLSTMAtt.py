...
class BiLSTMAtt(nn.Module):
    ...
    def forward(self, X):
        ...
        hidden_state = Variable(torch.randn(self.n_layer * 2, self.batch_size, self.hidden_dim)).cuda()
        cell_state = Variable(torch.randn(self.n_layer * 2, self.batch_size, self.hidden_dim)).cuda()
