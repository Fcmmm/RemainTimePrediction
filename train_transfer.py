        ...
        for start_pos_temp in range(start_pos,stop_pos):
            ...
            while epoch < max_epoch_num and learn_rate >= learn_rate_min:
                ...
                for (input, target) in data.train_batch:
                    ...
                    output = output.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                    target = target.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                    loss = criterion(output, target)
                    loss.backward(retain_graph=True)

                    loss = loss.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                    total_loss = total_loss.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
                ...
                if loss_change < 1 and len(loss_deque) == 20:
                    now_time = datetime.now().strftime('%Y-%m-%d(%H-%M-%S)')
                    model_save = save_model_folder + now_time + '.pth'
                    torch.save(model, model_save)
                    result_save_file = result_save_folder +'trans_' + 'length' + str(start_pos_temp) + 'epoch' + str(epoch) + now_time + '.csv'
                    ...
                    loss_deque = deque(maxlen=20)
                    loss_deque.append(total_loss.item())
                if len(loss_change_deque) ==50 and (max(loss_change_deque) - min(loss_change_deque) < 1):
                    now_time = datetime.now().strftime('%Y-%m-%d(%H-%M-%S)')
                    model_save = save_model_folder + now_time + '.pth'
                    torch.save(model, model_save)
